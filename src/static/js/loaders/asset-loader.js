(function (Loaders) {
  ...
  function AssetLoader (game, assets, textureAtlasJSONHash) {
    ...
    if (assets.atlases) {
      if (!textureAtlasJSONHash) {
        ...
        for (var k = 0; k < assets.atlases.length; k++) {
          game.load.atlas(assets.atlases[k].name, assets.atlases[k].image, assets.atlases[k].json, textureAtlasJSONHash);
        }
      }
    }
    ...
    // Load all fonts
    if(assets.fonts) {
      for(var f = 0; f < assets.fonts.length; f++) {
        game.load.bitmapFont(assets.fonts[f].name, assets.fonts[f].path, assets.fonts[f].fontPath);
      }
    }
